...
public class AdapterHoleDataMaintainNotYet extends BaseAdapter {
    ...
    public AdapterHoleDataMaintainNotYet(Activity a, ArrayList<HoleData> d) {
        this.mActivity = a;
        this.lstholedata = d;
        stt = 0;
    ...
    public View getView(int i, View view, ViewGroup viewGroup) {
        ...
        if (view == null) {
            ...
            vi = mInflater.inflate(R.layout.layout_adapter_holedata_maintain_not_yet, viewGroup, false);
            vi.requestFocus();
        ...
        try {
            if (lstholedata.size() <= 0) {
                holder.tvhole_route.setText("");
                holder.tvStreet_Name.setText("");

            } else {
                tempValue = null;
                tempValue = (HoleData) lstholedata.get(i);
                if(tempValue.getInspect_Status() >0){
                    holder.tvhole_route.setBackgroundResource(R.color.blue_kd);
                    holder.tvStreet_Name.setBackgroundResource(R.color.blue_kd);
                    holder.tvhole_name.setBackgroundResource(R.color.blue_kd);
                    holder.tvhole_address.setBackgroundResource(R.color.blue_kd);
                    holder.tvInspect_Day.setBackgroundResource(R.color.blue_kd);
                    holder.tvMaintain_Day.setBackgroundResource(R.color.blue_kd);
                    ...
                    holder.tvhole_route.setText(String.valueOf(i+1));
                    holder.tvStreet_Name.setText(String.valueOf(tempValue.getStreet_Name()));
                    holder.tvhole_name.setText(tempValue.getHole_Name());
                    holder.tvhole_address.setText(tempValue.getHole_Address());
                    holder.tvInspect_Day.setText(tempValue.getHoleType_Name());
                    holder.tvMaintain_Day.setText(tempValue.getHoleSize_Name());
                }
            }
